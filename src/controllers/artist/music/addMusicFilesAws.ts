...
export const addMusicFilesAws = async (req: Request, res: Response) => {
  ...
  if (!IsFindMusic) {
    throw new AppError(404, "Music is not found");
  }
      ...
      key: async (req: Request, file, cb) => {
        if (file.fieldname === "music") {
          if (file.mimetype === "audio/mpeg") {
            ...
            return res
              .status(400)
              .json({ message: "Music only in mp3 format" });
          }
        } else {
          if (file.fieldname === "image") {
            if (file.mimetype === "image/jpg") {
              ...
              return res
                .status(400)
                .json({ message: "Image can only be a png, jpg or jpeg" });
            }
          }
        }
      },
  ...
  return music(req, res, async () => {
    if (!saveNameMusic || !saveNameImage) {
      return res
        .status(400)
        .json({ message: "Music and image are required files" });
    }
